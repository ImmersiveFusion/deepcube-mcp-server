...
public class GeneralToolTests
{
    ...
    [Test]
    public async Task GetGridInfo_Returns_Valid_Json()
    {
        ...
        var result = await GeneralTool.GetGridInfo(_client, _anchor);
    ...
    [Test]
    public async Task GetGridInfo_Passes_Correct_GridSecondaryId()
    {
        _client.GetGridAsync(Arg.Any<Guid>()).Returns(Task.FromResult(new GridViewModel("g", null, "e", null)));

        await GeneralTool.GetGridInfo(_client, _anchor);

        await _client.Received(1).GetGridAsync(_anchor.GridSecondaryId);
    }

    [Test]
    public async Task GetGridInfo_Returns_Safe_Error_When_Client_Throws()
    {
